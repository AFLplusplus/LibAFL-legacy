...
/*
  Base template type for MapFeedback, it is not limited to arrays so that can be
  used with classes such as std::bitset
*/
template <typename MapType,
          typename MapBaseType,
          size_t map_size,
          typename ObvervationChannelType,
          auto ReduceFunction,
          MapBaseType init_value = 0,
          typename EntryMetaType = void>
...
Result<float> BaseMapFeedback<MapType,
  ...
  for (auto ob : executor->GetObservationChannels()) {
    if (auto hmob = dynamic_cast<ObvervationChannelType*>(ob)) {
      ...
      if (map_size < size)
        continue;  // maybe we should abort instead?
...
/*
  This is an unoptimized version without EntryMetaType. For the common cases,
  with u8 as MapBaseType, see the next specialization.
*/
template <typename MapType,
          typename MapBaseType,
          size_t map_size,
...
class BaseMapFeedback<u8[map_size],
                      u8,
                      map_size,
    ...
    if (auto hmob = dynamic_cast<HitcountsMapObservationChannel*>(ob)) {
      ...
      while (i--) {
        /* Optimize for (*current & *virgin) == 0 - i.e., no bits in current
           bitmap that have not been already cleared from the virgin map - since
        ...
        if (unlikely(*current) && unlikely(*current & *virgin)) {
          if (likely(ret < 2)) {
            ...
            /* Looks like we have not found any new bytes yet; see if any
               non-zero bytes in current[] are pristine in virgin[]. */
...
template <typename MapBaseType,
          ...
          MapBaseType init_value = 0,
          typename EntryMetaType = void>
class MapFeedback : public BaseMapFeedback<MapBaseType[map_size],
                                           MapBaseType,
                                           map_size,
                                           ObvervationChannelType,
                                           ReduceFunction,
                                           ...
                                           EntryMetaType> {
 public:
  MapFeedback() { std::fill_n(this->GetVirginMap(), map_size, init_value); }
};
...
template <typename MapBaseType,
          size_t map_size,
          typename ObvervationChannelType>
using MaxMapFeedback = MapFeedback<MapBaseType,
                                   map_size,
                                   ObvervationChannelType,
                                   ReducerMax<MapBaseType>>;

template <typename MapBaseType,
          size_t map_size,
          typename ObvervationChannelType>
using MinMapFeedback = MapFeedback<MapBaseType,
                                   map_size,
                                   ObvervationChannelType,
                                   ReducerMin<MapBaseType>,
                                   MapBaseType(-1)>;

template <size_t map_size>
using MaxMapFeedbackU8 =
    MapFeedback<u8, map_size, MapObservationChannel<u8>, ReducerMax<u8>>;

template <size_t map_size>
using MinMapFeedbackU8 = MapFeedback<u8,
                                     map_size,
                                     MapObservationChannel<u8, u8(-1)>,
                                     ReducerMin<u8>,
                                     u8(-1)>;

template <size_t map_size>
using MaxMapFeedbackU16 =
    MapFeedback<u16, map_size, MapObservationChannel<u16>, ReducerMax<u16>>;

template <size_t map_size>
using MinMapFeedbackU16 = MapFeedback<u16,
                                      map_size,
                                      MapObservationChannel<u16, u16(-1)>,
                                      ReducerMin<u16>,
                                      u16(-1)>;

template <size_t map_size>
using MaxMapFeedbackU32 =
    MapFeedback<u32, map_size, MapObservationChannel<u32>, ReducerMax<u32>>;

template <size_t map_size>
using MinMapFeedbackU32 = MapFeedback<u32,
                                      map_size,
                                      MapObservationChannel<u32, u32(-1)>,
                                      ReducerMin<u32>,
                                      u32(-1)>;

template <size_t map_size>
using MaxMapFeedbackU64 =
    MapFeedback<u64, map_size, MapObservationChannel<u64>, ReducerMax<u64>>;

template <size_t map_size>
using MinMapFeedbackU64 = MapFeedback<u64,
                                      map_size,
                                      MapObservationChannel<u64, u64(-1)>,
                                      ReducerMin<u64>,
                                      u64(-1)>;
