...
*/
uintptr_t fake__builtin_return_address(u32 depth) {
  return (uintptr_t)(void *)&fake__builtin_return_address;
}
...
void __afl_trace(const u32 x) {
  ...
  #if __GNUC__
  u8 c = __builtin_add_overflow(*p, (u8)1, p);
  *p += c;
  #else
  *p += 1 + ((u8)(1 + *p == 0);
  #endif
...
static void __afl_map_shm_fuzz() {
  ...
  if (id_str) {
    ...
    if (getenv("AFL_DEBUG")) { fprintf(stderr, "DEBUG: successfully got fuzzing shared memory\n"); }
...
static void __afl_map_shm(void) {
  ...
  if (__afl_final_loc) {
    ...
    if (__afl_final_loc % 8) __afl_final_loc = (((__afl_final_loc + 7) >> 3) << 3);
  ...
  if (getenv("AFL_DEBUG"))
    fprintf(stderr,
            ...
            id_str == NULL ? "<null>" : id_str, __afl_area_ptr, __afl_area_initial, __afl_map_addr, MAP_SIZE,
  ...
  if (id_str) {
    ...
    if (__afl_map_addr) {
      ...
      shm_base = mmap((void *)__afl_map_addr, __afl_map_size, PROT_READ | PROT_WRITE, MAP_FIXED_NOREPLACE | MAP_SHARED,
                      shm_fd, 0);
    ...
    __afl_area_ptr = mmap((void *)__afl_map_addr, __afl_map_size, PROT_READ | PROT_WRITE,
    ...
    free(__afl_area_ptr);
    __afl_area_ptr = NULL;
    if (__afl_final_loc > MAP_INITIAL_SIZE) __afl_area_ptr = malloc(__afl_final_loc);
    if (!__afl_area_ptr) __afl_area_ptr = __afl_area_initial;
  ...
  if (getenv("AFL_DEBUG")) { fprintf(stderr, "DEBUG: cmplog id_str %s\n", id_str == NULL ? "<null>" : id_str); }
  ...
  if (id_str) {
    ...
    /* map the shared memory segment to the address space of the process */
    shm_base = mmap(0, sizeof(struct cmp_map), PROT_READ | PROT_WRITE, MAP_SHARED, shm_fd, 0);
    if (shm_base == MAP_FAILED) {
...
static void __afl_start_snapshots(void) {
  ...
  if (__afl_map_size <= FS_OPT_MAX_MAPSIZE) status |= (FS_OPT_SET_MAPSIZE(__afl_map_size) | FS_OPT_MAPSIZE);
  ...
  if (__afl_sharedmem_fuzzing || (__afl_dictionary_len && __afl_dictionary)) {
    ...
    if (getenv("AFL_DEBUG")) { fprintf(stderr, "target forkserver recv: %08x\n", was_killed); }
    ...
    if ((was_killed & (FS_OPT_ENABLED | FS_OPT_AUTODICT)) == (FS_OPT_ENABLED | FS_OPT_AUTODICT) &&
  ...
  while (1) {
    ...
    if (!child_stopped) {
      ...
      if (!child_pid) {
        ...
        if (!afl_snapshot_take(AFL_SNAPSHOT_MMAP | AFL_SNAPSHOT_FDS | AFL_SNAPSHOT_REGS | AFL_SNAPSHOT_EXIT)) {
...
#ifdef __linux__
  if (/*!is_persistent &&*/ !__afl_cmp_map && !getenv("AFL_NO_SNAPSHOT") && afl_snapshot_init() >= 0) {
...
__attribute__((constructor(1))) void __afl_auto_second(void) {
  ...
  if (__afl_final_loc) {
    ...
    if (__afl_area_ptr && __afl_area_ptr != __afl_area_initial) free(__afl_area_ptr);

    if (__afl_map_addr)
      ptr = (u8 *)mmap((void *)__afl_map_addr, __afl_final_loc, PROT_READ | PROT_WRITE,
                       MAP_FIXED_NOREPLACE | MAP_SHARED | MAP_ANONYMOUS, -1, 0);
    else
...
void __sanitizer_cov_trace_pc_guard_init(uint32_t *start, uint32_t *stop) {
  ...
  if (getenv("AFL_DEBUG")) { fprintf(stderr, "Running __sanitizer_cov_trace_pc_guard_init: %p-%p\n", start, stop); }
...
void __cmplog_ins_hook1(uint8_t arg1, uint8_t arg2) {
  ...
  uintptr_t k = (uintptr_t)fake__builtin_return_address(0);
  k = (k >> 4) ^ (k << 8);
  k &= CMP_MAP_W - 1;
...
#else
void __sanitizer_cov_trace_const_cmp1(uint8_t arg1, uint8_t arg2) __attribute__((alias("__cmplog_ins_hook1")));
void __sanitizer_cov_trace_const_cmp2(uint16_t arg1, uint16_t arg2) __attribute__((alias("__cmplog_ins_hook2")));
void __sanitizer_cov_trace_const_cmp4(uint32_t arg1, uint32_t arg2) __attribute__((alias("__cmplog_ins_hook4")));
void __sanitizer_cov_trace_const_cmp8(uint64_t arg1, uint64_t arg2) __attribute__((alias("__cmplog_ins_hook8")));

void __sanitizer_cov_trace_cmp1(uint8_t arg1, uint8_t arg2) __attribute__((alias("__cmplog_ins_hook1")));
void __sanitizer_cov_trace_cmp2(uint16_t arg1, uint16_t arg2) __attribute__((alias("__cmplog_ins_hook2")));
void __sanitizer_cov_trace_cmp4(uint32_t arg1, uint32_t arg2) __attribute__((alias("__cmplog_ins_hook4")));
void __sanitizer_cov_trace_cmp8(uint64_t arg1, uint64_t arg2) __attribute__((alias("__cmplog_ins_hook8")));
...
void __sanitizer_cov_trace_switch(uint64_t val, uint64_t *cases) {
  ...
  for (uint64_t i = 0; i < cases[0]; i++) {
    ...
    uintptr_t k = (uintptr_t)fake__builtin_return_address(0) + i;
    k = (k >> 4) ^ (k << 8);
    k &= CMP_MAP_W - 1;
