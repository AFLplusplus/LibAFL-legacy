...
// The struct we will be using as the generic interface for the observation
// channel
...
u8 init_obs_channel(afl_observation_channel_t *obs_channel) {
  ...
  struct obs_interface interface[20] =
      (struct obs_interface *)obs_channel->interface;
...
u8 pre_run_call(afl_observation_channel_t *obs_channel) {
  ...
  // We're dealing with timeout observation here, so leaving this function out
  // for now.
  return 0;
...
u8 run_target(executor_t *super, u32 opt_one, void *opt_two) {
  ...
  executor_t *executor =
      (executor_t *)super;  // Since the super is the first item on the struct,
                            // this typecast is valid due to C standards
...
u64 reduce_feedback(afl_feedback_t *feedback, u64 old_value,
                    u64 proposed_value) {

  struct obs_interface *interface =
      (struct obs_interface *)feedback->obs_channel->interface;
  ...
  // Assuming the avg execution time has a score of 0.5, we scale other scores
  // based on that.
  for (int i = 0; i < 20; i++) {

    interface[i].fbck_score = 0.5 * (interface[i].exec_time / avg_exec_time);

    if (interface[i + 1].exec_time == 0) { break; }
...
int main() {
  ...
  // As our interface for the obs channel, we have an array of above created
  // struct
  struct obs_interface interface[20];
  ...
  for (int i = 0; i < 10; i++) {
    ...
    test_executor->super.executor_ops->place_inputs_cb(&test_executor->super,
                                                       mem, 80);

    test_executor->super.executor_ops->run_target_cb(&test_executor->super, 0,
                                                     NULL);
